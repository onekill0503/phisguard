...
contract SmartAccount is IERC7821, ISmartAccount {
    ...
    function execute(
        ...
        if (mode == ExecutionMode.CALL) {
            (success, result) = calls.to.call{value: calls.value}(calls.data);
        } else {
            (success, result) = calls.to.delegatecall(calls.data);
        }
    ...
    function updateAVS(address _avs) external {
        if (msg.sender != owner)
            revert FAILED_FORBIDDEN("Only owner can update AVS");
